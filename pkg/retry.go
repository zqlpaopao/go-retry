...
func (r *RetryManager) DoAsync(retryableFun RetryableFunc, CustomParams ...interface{}) {
	r.retryConf.pool.AddTask(&Task{
		f: func(retryableFunc RetryableFunc, i ...interface{}) {
			r.execute(retryableFun, CustomParams...)
		},
		retryableFun: retryableFun,
...
func (r *RetryManager) execute(retryableFun RetryableFunc, CustomParams ...interface{}) (uint, bool) {
	...
	for index <= r.retryConf.retryCount {
		if r.onRetryCallback != nil {
			r.onRetryCallback(index)
		}
		executeResult = retryableFun()
		if executeResult {
			break
		} else if index >= r.retryConf.retryCount {
			break
		}
		index++
	...
	if r.onCompleteCallback != nil {
		r.onCompleteCallback(index, executeResult, CustomParams...)
	}
